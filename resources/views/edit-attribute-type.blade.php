...
<html lang="en">
<head>
<title>@if(Auth::user()->user_type == 'vendor'){{ Helper::translation(2019,$translate) }}@else{{ Helper::translation(1912,$translate) }}@endif  - {{ $allsettings->site_title }}</title>
@include('style')
</head>
<body>
@include('header')
@if(Auth::user()->user_type == 'vendor')
    <section class="headerbg" style="background-image: url('{{ url('/') }}/public/storage/settings/{{ $allsettings->site_header_background }}');">
      <div class="container text-left">
        <h2 class="mb-0">{{ Helper::translation(2019,$translate) }}</h2>
        <p class="mb-0"><a href="{{ URL::to('/') }}">{{ Helper::translation(1913,$translate) }}</a> <span class="split">&gt;</span> <span>{{ Helper::translation(2019,$translate) }}</span></p>
      </div>
    </section>
  <main role="main">
    ...
    <div class="row">
        <div class="col-md-12">
             <form action="{{ route('edit-attribute-type') }}" class="setting_form" id="contact_form" method="post" enctype="multipart/form-data">
             {{ csrf_field() }}
                <div class="form-group row">
                    ...
                    <div class="col-sm-6">
                        ...
                        <select name="attribute_status" class="form-control" data-bvalidator="required">
                            <option value=""></option>
                            <option value="1" @if($edit['type']->attribute_status == 1) selected @endif>{{ __('Active') }}</option>
                            <option value="0" @if($edit['type']->attribute_status == 0) selected @endif>{{ __('InActive') }}</option>
                        </select>
                    </div>
                </div>
                <div class="form-group row">
